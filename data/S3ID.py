...
class S3ID(Dataset):
    ...
    def __init__(self, fname, root=MyPath.db_root_dir('s3id'), train=True, transform=None, sanomaly= None, mean_data=None, std_data=None):

        super(S3ID, self).__init__()
        self.root = root
        ...
        temp = np.asarray(temp.iloc[:, :])

        if np.any(sum(np.isnan(temp))!=0):
            print('Data contains NaN which replaced with zero')
            temp = np.nan_to_num(temp)

        self.mean, self.std = mean_data, std_data # actually min and max!
        ...
        if self.train:
            ...
            range_val = (std_data - mean_data) + 1e-20 # normalization
